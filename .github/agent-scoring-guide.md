...
각 축은 1-4 정수:
- **U (User value, 1-4)**: FOMO Club 사용자에게 실제 가치. 4=핵심 가치 직결, 3=눈에 띄는 개선, 2=marginal, 1=거의 무관
- **F (Feasibility, 1-4)**: 이번 주 **phase 1(단일 PR)** 로 구현 가능성. 큰 이니셔티브는 phase로 쪼갠 *첫 단계*가 단일 PR이면 만점 가능 — "크다"는 이유로 감점하지 않는다. 4=첫 phase 즉시 가능, 3=명확하지만 작업량, 2=설계 필요, 1=phase 분할 없이 통째로 커서 불확실
- **N (Novelty/Leverage, 1-4)**: 새로움 *또는* 레버리지. **강제 회전 폐지(2026-06-09)**: 같은 영역이라도 Target-Metric이 미달이라 *더 깊게* 파는 제안은 감점하지 않는다. 4=새 레버리지 큰 한 수 또는 미달 지표를 정조준한 심화, 3=다른 각도, 2=한계효용 적은 보완, 1=DONE(머지 완료) 영역 재탕 또는 근거 없는 반복
...
**제안 구조 권장 (전략 격상 — 2026-06-09)**: 단발 미시 수정 나열 대신, "북극성 대비 지금 가장 레버리지 큰 한 수"를 고른다. 크면 epic(phase 분할)으로 제안하되 이번 주 구현 대상은 첫 phase(단일 PR)다. PM은 로드맵/유저 저니, Designer는 화면·플로우 단위 UX까지 사고 범위를 넓힐 수 있다(구현 phase는 여전히 구체적으로).
