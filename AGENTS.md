...
**타로앱 사용자 경험 몰입감 완성이 유일한 우선순위다.**

- 애니메이션 강화, 인터랙티브 UX, 의식(ritual) 느낌 구현에 모든 에이전트가 집중
- 새 기능 제안/구현 시 "이게 몰입감을 높이는가?" 를 첫 번째 질문으로 삼는다
- 카카오톡 공유, 사주팔자, 어드민 UI 등 다른 피처는 이 지시가 해제될 때까지 후순위
