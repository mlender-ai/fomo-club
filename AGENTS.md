...
7. **Ship**: CI 그린 확인 후 **머지하고 프로덕션 배포까지 돌린다** — `gh workflow run vercel-production-deploy.yml -f ref=main`. 머지만 하고 멈추지 않는다. 예외는 아래 3원칙 3의 4가지뿐이고, 걸리면 사유를 보고한다.
...
### 🧭 세션 시작 체크리스트 (WO-SYNC E-2 — 전 에이전트 필수, 다른 무엇보다 먼저)

```
1. docs/STATUS.md 를 읽는다            ← 현재 상태의 정본
2. npm run verify:production          ← 정규 도메인 실측. exit 0 이 아니면 그 원인이 첫 과제다
3. 스케줄 워크플로·Vercel Cron 최근 실행을 확인한다
4. 메모리·세션 요약과 STATUS.md 가 어긋나면 STATUS.md 를 신뢰하고 오류를 보고한다
5. 지시서는 docs/wo/ 에서 읽는다. 채팅 원문만 있으면 먼저 커밋한다
```

**2번 종료코드**: `0` 통과 · `1` 실패(정규 도메인이 기대 상태가 아니다) · `2` 미확인(측정 못 함 —
**통과가 아니다**) · `3` 스크립트 자체 오류. 무엇을 재고 왜 그 셋이 실패조건인지는
`scripts/verify-production.ts` 헤더 주석에 있다. Vercel 자격증명 없이도 돌아간다.

**요약을 신뢰하지 않는다.** 인수인계 문서·세션 요약은 사실을 조금씩 변형시킨다 — 실제로
"WO-SUB 00~08 머지" 로 잘못 요약된 사례가 있었고(실측은 그렇지 않았다), 그것이 여러 세션에
걸쳐 승계됐다. 요약과 실측이 다르면 **실측이 이긴다.**
...
| **상태 갱신** | 매 WO 완료 시 **`docs/STATUS.md` 갱신을 완료 조건에 포함**한다 |
| **지시서 위치** | **`docs/wo/`**. 파일명 `WO-{배치}-{번호}-{영문슬러그}.md`, 한글 파일명 금지 |
| **번들 격리** | 신규 모듈은 **조회 경로 번들에 포함되지 않아야 한다.** 계산을 배치로 빼도 무거운 의존성이 조회 번들에 들어가면 콜드스타트가 나빠진다(실측: 콜드 49.0s vs 서버 self-report 175ms, 람다 10.15MB) |
| **감사 인증** | 읽기 전용 감사는 **`AUDIT_TOKEN`**. **`CRON_SECRET` 재사용 금지** — 크론 키는 쓰기를 부른다 |

### SSOT (모든 에이전트는 이 문서들을 읽고 그 안에서만 움직인다)
0. **`docs/STATUS.md` — 현재 상태 정본.** 아래 문서들이 "무엇을 만드는가"라면 이것은 "지금 어디인가"다.
...
3. **작업은 프로덕션까지 완결한다.** (2026-08-07 개정 — 종전 "실행은 광혁" 규정을 대체) 에이전트는 게이트 통과 후 **스스로 머지하고 프로덕션 배포까지 돌린다.** 머지된 채 배포 안 된 상태로 두지 않는다 — 사용자 화면에 닿지 않은 변경은 완료가 아니다. 방향·기획·카피 결정은 여전히 광혁 몫이고(3원칙 1), 바뀐 것은 **결정된 작업의 출고 책임**이다.
   - 배포: `gh workflow run vercel-production-deploy.yml -f ref=main` (백엔드 + fomo-web, 정규 도메인 alias까지)
   - **예외 — 멈추고 물어볼 것**: ① 크리티컬 이슈(게이트 실패·불변식 위반·제품 회귀 의심) ② prod DDL(`bootstrap_ledger`) ③ 유료 API 신규 결제 ④ 되돌리기 어려운 외부 노출(공개 게시·외부 발송)
   - 예외에 걸리면 배포를 건너뛰는 것으로 끝내지 않고 **무엇이 왜 막혔는지 보고**한다. 조용히 안 하는 것이 가장 나쁘다.
...
기준 기록: `docs/legacy/MASCOT.md`, `docs/legacy/FOMO_INDEX.md`, `docs/legacy/DESIGN_FOMO.md`. 현재 디자인 작업은 **`docs/design/DS-00_TOKENS.md`(기반 토큰 정본, v2)** → **`docs/design/DS-07_SHELL.md`(카드 껍데기·CTA·하단 바·넘김 흐름 정본)** → `docs/design/DS-0x_*.md`(화면별 스펙 — 현재 DS-01 메인 카드) → `docs/DESIGN.md`(그 외 서술) → `design/tokens.json`의 `ds` 블록(기계가독) → `docs/PRODUCT_VISION.md` 순으로 따른다. 상위가 이긴다. 정의 상세: `.claude/agents/mascot-keeper.md`.
