      ...
      <section style={{ marginBottom: 28 }}>
              ...
              {status.feed.symbols.map((s) => (
                <tr key={s.symbol}>
                  ...
                  <td className={s.status === "fresh" ? "up" : "down"}>
                    {s.status === "fresh" ? "정상" : s.status === "stale" ? "끊김" : "없음"}
                  </td>
        ...
        {!status.feed.ok ? (
          <p className="lab-empty-note">
            끊긴 채로 매매하면 성과가 거짓이 된다 — 페이퍼 실행기가 신규 진입을 멈춘다.
          </p>
        ) : null}
      </section>

      <section style={{ marginBottom: 28 }}>
        <p className="section-kicker" style={{ marginBottom: 8 }}>봉</p>
              ...
              {status.candles.map((c) => {
                ...
                return (
                  <tr key={`${c.symbol}-${c.interval}`}>
                    ...
                    <td className={gap && gap.missing > 0 ? "num down" : "num"}>
                      {gap ? `${gap.missing} (${gap.intervals}구간)` : "0"}
                    </td>
                    <td className="num">{c.last?.toISOString().slice(0, 16) ?? "—"}</td>
                  </tr>
                );
              })}
            </tbody>
        ...
        <p className="lab-empty-note">구멍은 메우지 않는다. 백테스트가 그 구간을 건너뛴다.</p>
      </section>

      <section>
        <p className="section-kicker" style={{ marginBottom: 8 }}>수집 잡</p>
            ...
            <tbody>
              {status.jobs.map((j) => (
                <tr key={j.job}>
                  ...
                  <td className={j.consecutiveFailures > 0 ? "num down" : "num"}>
                    {j.consecutiveFailures}
                  </td>
                  <td className="num">{j.finishedAt.toISOString().slice(0, 16)}</td>
                </tr>
              ))}
            </tbody>
          </table>
