...
할 때만 액션을 냅니다.`;
    ...
    // 컨텍스트 수집 (병렬) — 오픈 PR + 데일리 모니터 + 구현 실행 상태 + 피드백
    const [prs, monitorRuns, taskRuns, feedbackLog] = await Promise.allSettled([
      getOpenPRs(5),
      getWorkflowRuns("daily-product-monitor.yml", 5),
      getWorkflowRuns("implement-task.yml", 5),
      getRecentFeedbackLog(1500),
    ]);
    const feedbackSection =
      feedbackLog.status === "fulfilled" && feedbackLog.value ? feedbackLog.value : "(적재된 피드백 없음)";

    const latestMonitorRun =
      monitorRuns.status === "fulfilled"
        ? ((monitorRuns.value as { workflow_runs?: { status: string; conclusion: string | null; html_url: string; created_at: string }[] })
            .workflow_runs || [])[0]
        : undefined;
    const monitorStateLine = latestMonitorRun
      ? `상태=${latestMonitorRun.status} / 결론=${latestMonitorRun.conclusion ?? "진행중"} (${latestMonitorRun.created_at})`
      : "(실행 기록 없음)";
    ...
    const monitorRunList = monitorRuns.status === "fulfilled"
      ? (
          (monitorRuns.value as { workflow_runs: { conclusion: string; created_at: string; head_branch: string }[] })
            .workflow_runs || []
        )
          .map((r) => `- ${r.conclusion || "running"} (${r.head_branch}) — ${r.created_at}`)
          .join("\n")
      : "(조회 실패)";
    const taskRunList = taskRuns.status === "fulfilled"
      ? (
          (taskRuns.value as { workflow_runs: { conclusion: string; created_at: string; head_branch: string }[] })
            .workflow_runs || []
        )
          .map((r) => `- ${r.conclusion || "running"} (${r.head_branch}) — ${r.created_at}`)
          .join("\n")
...
## Daily Product Monitor 최신 실행 상태 (사실)
${monitorStateLine}

## 최근 Daily Product Monitor 실행 이력
${monitorRunList || "(없음)"}

## 최근 Implement Task 실행 이력
${taskRunList || "(없음)"}
...
- **개발/진행 상태(status)를 물으면 추측 금지. 오직 위 Daily Product Monitor / Implement Task 실행 이력과 오픈 PR 사실만으로 답한다.**
  - monitor가 success이고 critical 이슈/PR이 없으면 "오늘은 개발 스킵"이라고 말한다.
  - implement-task가 in_progress/queued일 때만 "구현 진행 중"이라고 말한다.
  - 실패/PR 미생성은 그대로 실패 또는 스킵이라고 말한다.
  ...
  try {
    switch (action.name) {
      ...
      case "implement": {
        ...
        const issue = await getLatestActionableIssue();
        if (!issue) {
          return "⚠️ 구현할 최신 작업 이슈가 없습니다. CEO Brief 기반 auto-implement는 기본 경로에서 제외했습니다. Daily Product Monitor가 critical 이슈를 만들면 자동 구현됩니다.";
        }
        await triggerWorkflow("implement-task.yml", { issue: String(issue.number) });
        return `🚀 *#${issue.number} 구현 시작* — 최신 작업 이슈로 판단했습니다: ${issue.title}\n${issue.html_url}`;
      }
