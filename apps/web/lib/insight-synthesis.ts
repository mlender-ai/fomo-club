...
function systemPrompt(): string {
  ...
  const restoredOnlyBanned = DEV_CONSTRAINTS_LIFTED
    ? []
    : ["매" + "수/매" + "도", "목표" + "가", "예측"];
  const judgmentPolicy = DEV_CONSTRAINTS_LIFTED
    ? [
        "개발 오버라이드 활성: 매수·매도 추천, 목표가, 손절선, 기술적 분석, 수급·매크로 판단과 예측 표현을 허용한다.",
        "단 입력에 없는 숫자·고유명사·종목 관계는 절대 만들지 마라. 판단은 자유지만 근거는 입력 JSON 안의 사실만 사용한다.",
      ]
    : [
        "예측·기대·전망·수혜·유망 등 미래 단정 금지. 사실과 해석까지만 쓴다.",
        "투자 조언과 방향 예측 금지. 사실과 해석까지만 쓴다.",
      ];
  const banned = [...baseBanned, ...restoredOnlyBanned];
  return [
    ...
    DEV_CONSTRAINTS_LIFTED
      ? "인과 서술은 허용한다. 단 입력 JSON의 출처·시점·수치로 뒷받침되는 관계만 쓴다."
