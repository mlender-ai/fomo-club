...
✅ **2026-09-08 복원 완료 (LAUNCH-P1 §C-1).** `ACTIVE: false` + `DEV_CONSTRAINTS_LIFTED = false`.
아래 §1 의 해제 목록은 **더 이상 발효되지 않는다** — 복원 절차(§4)를 위해 원문으로 보존한다.

### 왜 되돌렸나 — 실측

지시(광혁): *"게이트를 느슨하게 하지 않는다. override 가 실제로 필요했던 사례를 목록으로
보고하고, 정당한 사례가 없으면 문서에서 override 조항을 삭제한다."*

| 확인 | 결과 |
|---|---|
| 오늘 discovery 46장에서 진짜 조언 표현 | **0건** — 금칙어로 잡힌 36건은 전부 `순매수`(사실 용어)이고 게이트가 이미 중성화한다 |
| 과거 사례(`목표가 3만3000원 유지`, `목표가 5만5천원`) | **수집 기사 제목**이다. 우리가 쓴 문장이 아니다 |
| 그 문장이 사용자 화면에 나가나 | **아니다.** `HomeView` 는 `QuietPickDeck`(오늘의 조용한 돈)만 그린다 — 레거시 발견 피드는 props 로 받고 쓰지 않는다 |
| 지금 서빙 페이로드의 `목표가` | 0건 |

**정당한 사례가 없다** → 조항을 걷었다. 게이트 둘(`spec-analyze` · `discovery-regression-gate`)이
이제 같은 규칙으로 돈다 — 어긋남이 STATUS §31 의 버그였다.
