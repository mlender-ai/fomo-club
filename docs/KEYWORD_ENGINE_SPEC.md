...
| **선행 문서** | `docs/PRODUCT_TRUTH.md`(제품 정의·North Star), `docs/FOMO_INDEX.md`(점수 철학) |
| **버전** | v1 |

> 이 문서는 "오늘 사람들 시선이 가장 쏠린 키워드"를 **실제 데이터로** 산출하는 엔진의 개발 스펙이다.
> 화면(틴더 스와이프 카드 + 깊이 페이지)은 이미 있다. 빠진 것은 그 뒤의 데이터 엔진이다.
