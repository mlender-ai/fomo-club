...
> ⚠ 현재 `docs/CONSTRAINT_OVERRIDE_DEV.md`(ACTIVE)가 발효 중 — 개발 단계 동안 **투자조언·예측·표현 제약은 해제**, **사실 정확성 제약만 유지**. 본 문서의 해당 표현 제약은 오버라이드됨(개별 제약 문구는 복원 위해 원문 보존). 출시 전 ACTIVE:false 로 복원.

> ⚠️ **[폐기 — 실행 금지·히스토리 보존]** 감정 투표 MVP 시기의 일회성 구현 프롬프트. 아래 Phase 명령을 다시 실행하지 않는다. 현재 정본: `docs/PRODUCT_VISION.md` (**주식시장의 틴더 v5 — 두 표면·한 엔진**)와 루트 `CLAUDE.md`. ("시장 참여자 감정 시각화"는 폐기된 정체성.)
...
## 1. docs/legacy/FOMO_CLUB.md 신규 생성
...
- 상세는 docs/legacy/FOMO_INDEX.md 참조.
...
## 2. docs/legacy/FOMO_INDEX.md 신규 생성
...
### FOMO Club
docs/legacy/FOMO_CLUB.md, docs/legacy/FOMO_INDEX.md 참조. 모노레포 내 별도 앱(apps/fomo-club, apps/fomo-web).
...
[포모 마스코트 (메인) + FOMO Index 숫자 (보조)]   ← docs/legacy/MASCOT.md 기준
