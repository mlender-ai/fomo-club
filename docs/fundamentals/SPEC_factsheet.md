...
- **`filed_at_source`** 로 근거를 구분한다(3값). 레코드와 `fiscal` 롤업 양쪽에 남긴다.
  - `disclosed` — 소스가 실제 공시일을 준 경우(SEC `filed`, **DART `rcept_no` 앞 8자리**)
  ...
  - `none` — 공시일 근거가 전혀 없다. 레코드 단위에서는 도달하지 않고(그런 분기는 만들지 않는다),
    `fiscal` 롤업에서 "분기 레코드 0개" 를 뜻한다.
- **롤업은 보수적으로 내려 잡는다.** 분기 중 하나라도 `statutory_deadline` 이면 `fiscal.filed_at_source`
  전체가 `statutory_deadline` 이다. 섞여 있을 때 `disclosed` 로 표시하면 추정 공시일 위에서 계산된
  밴드를 실측으로 착각하게 되고, 그 위에 세운 `WO-SUB-07` 채점이 오염된다.
- **출처가 바뀌면 밴드를 다시 계산해야 한다.** `statutory_deadline` → `disclosed` 승격은 단순
  재백필이 아니다. look-ahead 가드가 추정 공시일 위에서 돌았으므로 시점 `d` 의 TTM 구성이 실제와
  달랐을 수 있다. 팩트시트를 통째로 재조립해 밴드까지 다시 만든다(해시가 바뀌므로 새 스냅샷이 남고,
  `filed_at_source` 로 이전 값과 구분된다).
...
| 공시일 | **없다** → `filed_at_source: "statutory_deadline"`. DART 가 열린 뒤로는 네이버가 재무 1차 소스가 아니다(§3-5) |
...
### 3-5. KR — DART (열림, 재무 1차 소스)

`DART_API_KEY` 는 **Vercel 환경변수에 있고 작동한다**. 앞선 판의 이 절은 "미착수 — 키 없음" 으로
적혀 있었는데 그것이 틀렸다: 로컬·GH Actions 에서 키를 **읽을 수 없는 것**과 DART 를 **쓸 수 없는 것**은
다르다. 크론 라우트가 그 키를 가진 실행 환경이다. 구조 덤프 근거는 `docs/fundamentals/DUMP_dart_structure.md`.

**DART 가 KR 재무의 1차 소스다.** 네이버는 시세·컨센서스·업종·일별 종가를 계속 담당한다(DART 에 없다).

#### 실측 함정 — 이것을 모르면 조용히 가짜 숫자가 된다

| 함정 | 실측 |
|---|---|
| **분기 손익은 누적이 아니다** | `thstrm_dt` 는 `"2025.01.01 ~ 2025.06.30"` 으로 **누적처럼 표기**하지만 `thstrm_amount` 는 **그 분기 3개월치**다. 종근당 2025: 1분기 400,955,846,488 / 반기 434,849,104,888(= Q2 만, 2×Q1 아님) / 3분기 429,818,748,658 / 연간 1,692,403,987,134 |
| 보고서 → 분기 대응 | `11013`→Q1 · `11012`→**Q2** · `11014`→Q3 · `11011`→연간. **Q4 = 연간 − (Q1+Q2+Q3)**, 세 분기 중 하나라도 없으면 구성하지 않는다(부분 합으로 빼면 과대계상) |
| 연결·개별이 한 응답에 | 주요계정 응답에 같은 `매출액` 행이 **두 번** 온다(`fs_div: CFS` / `OFS`). 필터하지 않으면 응답 순서에 의존한다. **연결 우선, 섞지 않는다** |
| `account_id` 는 전체 재무제표에만 | 주요계정(`fnlttSinglAcnt`)에는 `account_id` 필드가 **없다**. 8분기 경로가 여기라서 계정명 매칭이 불가피하다 |
| 전체 재무제표는 있다고 가정 불가 | 당해 사업보고서가 `013`(미제공)일 수 있다(같은 조합의 주요계정은 정상). 013 이면 현금흐름·EPS 는 그 보고서에서 `null` — **이전 분기로 메우지 않는다**(INV-12) |
| `corp_code` 매핑 | `corpCode.xml`(ZIP) 뿐. `list.json` 경로는 `status 100`(3개월 제한)으로 막힌다. 전송이 **238초**라 캐시 필수(7일) |
| EPS | `ifrs-full_BasicEarningsLossPerShare` = **기본** EPS. 희석으로 옮겨 적지 않으므로 KR `eps_diluted` 는 `null` 유지 |
| 감가상각비 | 단독 계정이 표본에 없다(비현금항목의 조정에 합산) → **KR EV/EBITDA 는 `null`** |
| 기간말 | 재무상태표 행의 `thstrm_dt`(`"2025.03.31 현재"`)에서 읽는다. 손익 행은 누적 기간 표기라 분기말이 아니다 |

계정 매핑 정본: `packages/fomo-core/src/fundamentals/dart-accounts.json`(`mapping_version` 을 찍는다 —
계정 체계가 바뀌면 과거 파싱 결과의 의미가 달라지므로 버전 없이 고치지 않는다).

#### DART 가 풀어준 것

| 이전 상태 | DART 이후 |
|---|---|
| KR 분기 5개 상한(네이버) | 연도를 올려 부르면 상한 없음. 한 해 = 분기 3개 + Q4 구성 1개 |
| KR 연간 3개 | 사업보고서 한 응답이 당기·전기·전전기 3개 연도를 준다 |
| `revenue_cagr_3y` KR 0/40 | 연간 4개 확보 가능 |
| `operating_stdev_8q`·`trend_8q` | 8분기 확보 → 시클리컬 판정 활성(02R 재검토) |
| 밴드 백분위 유효 관측 10~25% | `filed_at_source: "disclosed"` 로 승격 |
| 현금·차입금·이자지급·영업현금흐름 | 전체 재무제표에서 확보(`sj_div: CF` 32~33행 실측) |
