...
### 레인이 둘인 이유 — 시세가 남의 뒤에 서면 안 된다

처음에는 전부 한 줄로 세웠다. **레인 안에서 한 번에 하나만** 도는 것은 맞다 — 병렬이면
DB 커넥션이 몰리고(서버리스는 람다당 1개다) 어느 잡이 느린지도 안 보인다.

그런데 FCE 업로드가 길어지자 시세가 그 뒤에서 **94초까지 밀렸다**(실측). 업로드가
타임아웃(120초)까지 가면 시세 간격이 `STALE_AFTER_MS`(3분)를 넘긴다. 그러면
**수집은 멀쩡한데 화면에 `끊김` 이 뜬다** — 이 러너가 없애려고 만들어진 바로 그 화면이다.

그래서 `latest-price` 만 **혼자 도는 레인**에 둔다. 3심볼 한 번 GET 에 upsert 3행이라
커넥션을 잡아먹는 쪽이 아니다. 나머지는 한 줄로 선다.

실패는 콘솔에만 남기지 않고 `CollectionRun` 에 같이 적는다 — **성공만 적으면 언제부터
안 들어왔는지 모른다.**
...
| 워크플로 | 크론 | 무엇 |
|---|---|---|
| `lab-collect` | 4개 | 시세(5분) · 봉(매시 5분) · 고래(15분) · 페이퍼(매시 15분) |

**펀딩비 크론은 껐다.** 거기서는 구조적으로 못 받는다 — `fapi.binance.com` 이 GitHub
러너(미국 IP)에 451 을 준다. 끄기 전까지 **연속 실패 13회**가 쌓이면서 `/data` 에
`펀딩비 ❌` 가 떴는데, 정작 펀딩비는 로컬 러너가 받고 있었다. **화면이 거짓말을 한 것이다.**
파일은 지우지 않았고 `workflow_dispatch` 로는 여전히 돌릴 수 있다.
