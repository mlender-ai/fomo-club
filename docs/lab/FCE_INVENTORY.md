...
**기본값은 3배다.** 화면에 보이는 10배는 환경변수로 덮은 값이다 —
레포만 읽어서는 실제 운용 배수를 알 수 없고, **포지션마다 저장된 값을 화면에 띄워야 한다.**

MDD 35.7% 도 그 배수에서 나온 숫자다. 배수를 안 적으면 낙폭이 전략 탓으로 읽힌다.
...
| 펀딩비 | `fapi.binance.com` HTTP 451 — 랩 쪽 `lab-collect` 의 `funding` 잡도 같은 이유로 실패중 |
