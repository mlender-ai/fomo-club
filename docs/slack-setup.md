# FOMO Club Agent — Slack 설정 가이드

## 개요

FOMO Club Agent은 Slack에서 자율 파이프라인을 제어하는 봇입니다.
- **Slash Command** (`/fomo`) — 파이프라인 제어 (implement, status, merge 등)
- **Mention** (`@FOMO Club Agent`) — GPT-4o 기반 자유 대화 + 커맨드
...
3. 입력:
   ...
   - **Short Description**: `FOMO Club 파이프라인 제어`
...
| `GITHUB_REPO` | `mlender-ai/fomo-stock-app` | |
...
- **Repository access**: `mlender-ai/fomo-stock-app`
- **Permissions**:
...
4. `/invite @FOMO Club Agent` 입력하여 봇 초대
...
```
@FOMO Club Agent 현재 파이프라인 상태 알려줘
```
→ GPT-4o가 컨텍스트 기반으로 답변

```
@FOMO Club Agent 다음에 뭐 구현해야 해?
```
→ CEO Brief 기반 추천
...
| 커맨드 | 설명 |
|--------|------|
| `/fomo implement {YYYY-MM-DD}` | CEO Brief 기반 자동 구현 트리거 |
| `/fomo council` | Daily Agent Council 수동 실행 |
| `/fomo status` | 오픈 PR + CEO Brief + 실행 상태 요약 |
| `/fomo approve {이슈#}` | 이슈에 `implement-approved` 라벨 추가 |
| `/fomo merge {PR#}` | PR squash 머지 |
| `/fomo help` | 도움말 |

멘션 (`@FOMO Club Agent {질문}`) 은 자유 대화로 처리됩니다.
...
```
Slack User
  ├── /fomo {command}  →  POST /api/slack/commands  →  dispatchCommand()
