 ...
 *
 * ## ⚠️ 이 예산은 **메인 카드의 것이 아니다** (2026-08-23)
 *
 * WO-HOOK-01 이 DS-01 을 대체하면서 메인 카드의 후킹은 `hook` 19px 2줄(줄당 약 15자)이 됐고,
 * 근거 한 줄은 보조 2줄로 바뀌었다. **메인 카드의 폭 예산은 이제
 * `keyword-cards/card-type.ts` 가 문안을 짧게 만드는 것으로 지키고, 그 계약은
 * `card-type.test.ts` 의 "완료 기준 12" describe 가 형·변형 전수로 고정한다.**
 *
 * 이 모듈은 아직 `buildQuietPickHook` 계열(발견 덱 카드·상세 히어로 등 24px 훅을 쓰는 자리)의
 * 예산으로 남아 있다. 메인 카드 훅에 이 예산을 적용하지 말 것 — 폰트가 달라 줄당 문자수가 다르다.
 *
 * ## DS-01 이전과 달라진 점
 *
