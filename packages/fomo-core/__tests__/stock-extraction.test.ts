...
describe("resolveStock — 종목 검증 게이트(MLCC·HBM 등 비종목어 차단, 품질 최우선)", () => {
  it("MLCC·HBM·온디바이스AI 같은 부품·기술·소재어는 종목으로 해석 안 됨(null)", () => {
    for (const t of ["MLCC", "HBM", "온디바이스AI", "온디바이스 AI", "반도체", "AI반도체", "패키징"]) {
      expect(resolveStock(t)).toBeNull();
      expect(isVerifiedStock(t)).toBe(false);
    }
  });
  it("실제 상장 종목은 표준명으로 해석(별칭·공백·대소문자 허용)", () => {
    expect(resolveStock("마이크론")?.canonical).toBe("마이크론");
    expect(resolveStock("하이닉스")?.canonical).toBe("SK하이닉스"); // 별칭 → 표준명
    expect(resolveStock("삼성전기")?.canonical).toBe("삼성전기");
    expect(resolveStock("DB하이텍")?.canonical).toBe("DB하이텍");
    expect(resolveStock(" nvda ")?.canonical).toBe("엔비디아"); // 티커·공백
  });
  it("'삼성전자 관련주' 처럼 수식 붙은 건 종목으로 둔갑 안 함(정확/별칭 동치만)", () => {
    expect(resolveStock("삼성전자 관련주")).toBeNull();
    expect(resolveStock("반도체 수혜주")).toBeNull();
  });
});
