 ...
 *
 * ## 상한 7% — 감이 아니라 잰 분포로 골랐다
 *
 * 창을 바꾼 코드를 먼저 배포하고, **조건을 걸지 않은 채** 순변동 분포를 쟀다
 * (거래량 2배 이상 8종목 대상, 2026-08-26 프로덕션):
 *
 * ```
 * 순변동 ≤3%  →  0종목
 * 순변동 ≤5%  →  0종목
 ...
 * ```
 *
 * WO 가 준 범위는 3~7% 다. 그 범위 안에서 **카드가 하나라도 나오는 값은 7% 뿐이다.**
 * 5%(범위의 가운데)를 잠정으로 뒀다가 이 숫자를 보고 7%로 확정한다 — 범위를 넘겨 늘리지는
 * 않는다. 더 필요하면 조건을 푸는 게 아니라 **유니버스를 넓힌다**(PART D). 그래야
 * "이미 오른 종목"이 섞이지 않는다.
 */
export const VOLUME_AWAKENING_MAX_MOVE_PCT = 7;
